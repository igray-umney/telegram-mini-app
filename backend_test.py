    ...
    def test_test_notification(self):
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                data = response.json()
                if "chat not found" in data.get("error", ""):
                    self.log_test("Test Notification", True, "Expected error: User not registered with bot", data)
                    return True
                else:
                    self.log_test("Test Notification", False, f"Unexpected 500 error: {data}", data)
                    return False
            else:
    ...
    def test_create_stars_invoice(self):
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                data = response.json()
                if "chat not found" in data.get("message", ""):
                    self.log_test("Create Stars Invoice", True, "Expected error: User not registered with bot", data)
                    return True
                else:
                    self.log_test("Create Stars Invoice", False, f"Unexpected 500 error: {data}", data)
                    return False
            else:
